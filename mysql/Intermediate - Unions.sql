...
-- 这样操作会导致occupation和salary分别混入到first_name和last_name中，从而无法正确展示信息 
...
-- ‘old’标记表示起别名，与aliase不同，这个别名可以展示在查询表中，作为一个标签label战士 
SELECT first_name, last_name, 'Old'
FROM employee_demographics
WHERE age > 50;


-- label扩展应用，组合多个要删除的项目 
