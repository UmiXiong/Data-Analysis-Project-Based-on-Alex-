...
-- 关键词procedure用于创建一个指定的操作程序，存储在stored procedures下
...
-- 如果要调用指定的程序，直接使用关键字call
-- call parks_and_recreation.large_salaries()，可以这么干，但是没有必要，因为选中的数据库就是这个
CALL large_salaries();
...
-- 这么做会导致第二个select直接变成随机查询，结果就是无事发生
...
-- 使用关键词delimiter $$，这样我们就可以进行多重查询了；以delimiter开始，以delimiter结束
-- 如果是多段语句，使用begin...end 
DELIMITER $$
...
SHOW PROCEDURE STATUS WHERE Name = 'large_salaries3';
...
-- it automatically adds the dilimiter for us
-- 类似于函数，通过指定指定的参数获取结果, 这里指定的参数为整数类型，推荐在参数后面加param标记为参数
DELIMITER $$
