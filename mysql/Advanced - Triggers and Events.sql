...
-- 格式与stored producers类似
-- trigger类似于一个程序，满足特定条件即可触发
USE parks_and_recreation;
DELIMITER $$

-- 创造一个trigger，要求每当employee_salary加入新的数据后，就把相应的数据打入employee_demographics 
-- 显示在表的triggers目录下
...
-- 使用关键字new表示插入新的数据，对应after的操作，同理还有old，表示使用旧的数据
...
-- 这里就是一个实际案例，往salary插入数据之后，demographics中也会有对应的数据
