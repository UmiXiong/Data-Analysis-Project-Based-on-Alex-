...
-- 默认情况下，join为inner join, 此处要求id相同 （如果直接输入employee_id，则结果会非常模糊）
SELECT *
FROM employee_demographics
...
-- 排除最后一行（on...)，这里的行数为表1行数*表2行数，列数则是表1列数+表2列数
-- use aliasing!
SELECT *
...
-- 左连接保留左侧表的所有内容，即使右侧表没有任何内容；而右连接保留右侧表的所有内容，即使左侧表没有任何内容
-- 左连接保留语句靠前的表，右连接保留语句靠后的表
SELECT *
...
-- 多重聚合 
-- 即使demo和departments没有相同的关联项，但是依然可以通过salary表进行关联 
SELECT *
FROM employee_demographics dem
INNER JOIN employee_salary sal
	ON dem.employee_id = sal.employee_id
